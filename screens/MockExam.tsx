...
const MockExam: React.FC = () => {
  ...
  const handleStartExam = async (config: ExamConfig) => {
    ...
    // 1. Tải toàn bộ câu hỏi của các môn trong kỳ thi ngay từ đầu
    for (const sc of config.subjects) {
      const subjectQ = await db.getQuestionsBySubjectRecursive(sc.subjectId);
      const shuffled = subjectQ.sort(() => 0.5 - Math.random()).slice(0, sc.count);
    ...
    const firstSubjectQuestions = allQuestions.slice(0, firstSubjectConfig.count);
